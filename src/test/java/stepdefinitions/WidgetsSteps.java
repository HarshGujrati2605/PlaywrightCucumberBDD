...
public class WidgetsSteps {
    ...
    @When("user clicks {string} navbar option in Widgets page")
    public void clickElementNavBar(String navbarOption) {
        widgetsPage.clickWidgetsNavBar(navbarOption);
    }

    @When("user enters {string} in Single color name edit box in Auto Complete section in Widgets page")
    ...
    @Then("verify {string} tooltip is displayed in Tool Tips section in Widgets page")
    public void verifyTooltip(String toolTip) {
        Assert.assertTrue(widgetsPage.verifyTooltip(toolTip));
    }

    @When("user selects {string} in Old Style Select Menu in Select Menu section in Widgets page")
